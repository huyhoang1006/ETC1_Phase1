...
@section('pageTitle', 'Rơ le - Thống kê hãng thiết bị')
            ...
            <div class="col-md-12">
                <div class="breadcrumb-list">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">Rơ le - Thống kê hãng thiết bị</li>
                    </ol>
                </div>
            </div>
        </div>
                    ...
                    <div class="card m-b-30">
                        ...
                        <form class="form card-body module_search" style="align-items: flex-end">
                            <div class="form-group form_input">
                                <p>Thiết bị</p>
                                <select class="select2-single form-control" name="types[]" id="" multiple="multiple">
                            ...
                            <div class="form-group form_input">
                                <p>Chủng loại</p>
                                <select class="select2-single form-control" name="zCI_Device_Types[]" id="" multiple="multiple">
                                    @foreach ($deviceTypes as $deviceType)
                                        <option value="{{ $deviceType['zsym'] }}">{{ $deviceType['zsym'] }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group form_input">
                                ...
                                <select class="select2-single form-control" name="zrefnr_dvql_ids[]" multiple="multiple">
                                    @if (!empty($managementUnit))
                                        @foreach ($managementUnit as $val)
                                            <option value="{{ $val['id'] }}" {{ !empty($request['zrefnr_dvql_ids']) && in_array($val['id'], $request['zrefnr_dvql_ids']) ? 'selected' : '' }}>{!! $val['zsym'] !!}</option>
                                        @endforeach
                                    @endif
                                </select>
                            </div>
                            <div class="form-group form_submit" style="display: flex;align-items: center;width: 20%;">
...
@push('scripts')
    <script>
        $('.module_search').submit(function(e){
            ...
            $.ajax({
                url: '{{ route('admin.equipmentManufacturerStatisticsReportPreview') }}',
                type: 'POST',
                dataType: 'html',
                data: $(this).serialize()

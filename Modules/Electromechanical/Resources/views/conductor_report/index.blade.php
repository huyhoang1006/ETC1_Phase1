...
@section('content')
    <div class="row dashboard-index detail-report" style="padding-top: 0px">
        <div class="col-lg-12 col-xl-12">
            <h4 class="m-b-30">Báo cáo thống kê cáp và dây dẫn đã thí nghiệm - Báo cáo tổng quan</h4>
        </div>
        <div class="col-sm-12 col-xl-12">
            <div class="card m-b-30">
                ...
                <form class="card-body module_search" style="padding-top: 15px;padding-bottom: 20px;">
                    ...
                    <div class="form-group form_input form_type" style="position: relative;">
                        <p>Loại dây dẫn</p>
                    ...
                    <div class="form-group form_input">
                        <p>Đơn vị sử dụng</p>
                        <select class="select2-single form-control" name="management[]" multiple="multiple">
                            @foreach($management_unit as $val)
                                <option value="{!! $val['zsym'] !!}">{!! $val['zsym'] !!}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group form_input">
                        <p>Hãng sản xuất</p>
                        <select class="select2-single form-control" name="zManufacturer[]" multiple="multiple">
                            @foreach($manufactures as $val)
                                <option value="{{ $val['id'] }}">{!! $val['zsym'] !!}</option>
                            @endforeach
                        </select>
                    </div>
...
@push('scripts')
    <script type="text/javascript">
        ...
        $(".module_search").submit(function(e) {
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('admin.exportConductorStatisticsReport') }}",
                dataType: 'html',
                data: $(this).serialize(),

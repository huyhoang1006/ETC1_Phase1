...
class PetrochemicalManufacturesExport implements FromView, WithEvents
{
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class    => function(AfterSheet $event) {
                ...
                $event->sheet->getStyle('B3:D1000')->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_LEFT);
                $event->sheet->mergeCells('A3:D3')->getCell('A3')->setValue('Báo cáo thống kê hãng sản xuất');
    ...
    public function view(): View
    {
        ...
        $result = petrochemicalManufacturesData($request);

        if (!empty($result['error'])) {
            return view('report::petrochemicalManufactures.export', [
                'error' => $result['error']
            ]);
        }
